...
- Vault health check's `apply_selected` button already swapped in a spinner while applying changes, but `disabled:opacity-50` dimmed the whole button (spinner included) to half brightness, reading as "grayed out and stuck" rather than "working." It now stays at full opacity and shows `applying...` while in flight.
