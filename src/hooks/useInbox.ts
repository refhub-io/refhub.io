...
export function useInbox() {
  ...
  const updateItemHints = useCallback(async (
    ...
    await supabase.from('inbox_items').update(hints).eq('id', id);
  ...
  const rejectItem = useCallback(async (id: string) => {
    await supabase.from('inbox_items').update({ status: 'rejected' }).eq('id', id);
    setItems((prev) => prev.filter((item) => item.id !== id));
  }, []);
  ...
  const postponeItem = useCallback(async (id: string) => {
    ...
    await supabase.from('inbox_items').update({ sort_order: nextSortOrder }).eq('id', id);
    setItems((prev) => {
